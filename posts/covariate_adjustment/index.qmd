...
citation:
  type: post
  title: How to get the most out of prognostic baseline variables in clinical trials
  author: Courtney Schiffman, Michel Friesenhahn and Christina Rabe
...
In drug development, the FDA has provided a [draft guidance
...
Senn, the figure above shows the conditional type 1 error as a function of
...
In a 2021 [draft
guidance](https://www.fda.gov/regulatory-information/search-fda-guidance-documents/adjusting-covariates-randomized-clinical-trials-drugs-and-biological-products),
pre-specified covariate adjustment was fully supported and encouraged by the FDA
as a primary endpoint analysis in clinical trials:

> "Although an unadjusted analysis is acceptable for the primary analysis,
> adjustment for baseline covariates will generally reduce the variability of
> estimation of treatment effects and thus lead to narrower confidence intervals
> and more powerful hypothesis testing" [@FDA]. 

Importantly, the guidance states that "[s]ponsors should prospectively specify
the covariates and the mathematical form of the covariate adjusted estimator"
[@FDA]. We interpret this to mean that the exact working regression model form
...
Table 1 provides instructions and code for estimating the Treatment Arm Means
...
standardized regression estimator because it is easy to use, "statistically
reliable" [@FDA], has comparable power and can also be used to estimate TAMS and
...
In the example code in Table 1, $X$ is a single covariate, but $X$ can also be a
...
In case of missing values in baseline covariates, the FDA draft guidance
suggests that "covariate adjustment is generally robust to the handling of
subjects with missing baseline covariates. Missing baseline covariate values can
be singly or multiply imputed, or missingness indicators (Groenwold et al. 2012)
can be added to the model used for covariate adjustment. Sponsors should not
perform imputation separately for different treatment groups, and sponsors
should ensure that imputed baseline values are not dependent on any
post-baseline variables, including the outcome" [@FDA]. A baseline covariate
...
the superpopulation framework and are deterministic. 
...
re-sampling based methods like the non-parametric bootstrap. In their recent
guidance, the FDA recommends using the Huber-White robust “sandwich” estimator
instead of nominal standard error estimates from statistical software [@FDA].
However, if using a working regression model with treatment-by-covariate
...
In this case, the FDA recommends that the standard error estimation *"account
for the stratified randomization (Bugni et al. 2018) with or without strata
variables in an adjustment model. Otherwise, the standard error is likely to be
overestimated and interval estimation and hypothesis testing can become unduly
conservative"* [@FDA].
...
The figure below shows the estimated mean changes in GA lesion size with and
...
**Roche GA Development Team**: Neha Anegondi, Lee Honigberg, Simon Gao, Qi Yang,
Daniela Ferrara, Julia Cluceru, Verena Steffen
...
```{r hws_est_superpopulation, eval = T, echo = TRUE}
library(dplyr)
library(ggplot2)
library(sandwich)
library(doParallel)
